...
$client->post(
    $uploadUrl . '/assets?name=dts-trading.zip',
    array('Content-Type' => 'application/zip'),
    fopen(__DIR__ . '/artifacts/dts-trading.zip', 'r')
)->send();

$client->post(
    $uploadUrl . '/assets?name=dts-trading.phar',
    array('Content-Type' => 'application/phar'),
    fopen(__DIR__ . '/artifacts/dts-trading.phar', 'r')
)->send();
